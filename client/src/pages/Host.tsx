...
import {
  ...
  reapplyQualitySettings,
  requestKeyFrame,
  setupOptimizedCandidateHandler,
  addVideoTrackWithSimulcast,
  enableOpusFecDtx,
  setPlayoutDelayHint,
  restartICE,
  forceH264OnlySDP,
...
export default function Host() {
  ...
  // Co-host state
  const [cohostQueue, setCohostQueue] = useState<
  ...
  const qualityManagers = useRef<Map<string, AdaptiveQualityManager>>(
    new Map()
  );
  const monitoringCleanups = useRef<Map<string, () => void>>(new Map());
  const candidateHandlerCleanups = useRef<Map<string, () => void>>(new Map());
  const viewerPlatforms = useRef<Map<string, { isIOSSafari: boolean }>>(
    new Map()
  );
      ...
      ws.onmessage = async event => {
        ...
        if (msg.type === 'participant_count_update') {
          ...
          (msg.type === 'joined_stream' ||
            ...
            msg.type === 'viewer_join') &&
          msg.userId
        ) {
          console.log('[HOST] viewer joined:', msg.userId);
          // Store viewer platform info for codec preference
          viewerPlatforms.current.set(msg.userId, {
            isIOSSafari: msg.isIOSSafari || false
          });
          ...
          if (isLive && localStreamRef.current) {
            ...
            // Host isn’t live yet (or no tracks) → queue for later
            ...
            for (const transceiver of transceivers) {
              if (
                ...
                if (params.codecs && params.codecs.length > 0) {
                  ...
                  const viewerPlatform = viewerPlatforms.current.get(
                    msg.fromUserId
                  );
                  console.log(
                    `✅ Codec selected for viewer ${msg.fromUserId.substring(
                      0,
                      8
                    )}: ${codec} (isIOSSafari: ${viewerPlatform?.isIOSSafari})`
                  );
                }
                break;
              }
          ...
          const pc =
            viewerPcs.current.get(msg.fromUserId) || guestPcRef.current;
          if (pc && msg.candidate) {
            // Log first ICE candidate once per connection
            if (!haveLoggedFirstIce.current.get(msg.fromUserId)) {
              console.log(
                '[HOST] First ICE candidate received from',
                msg.fromUserId
              );
          ...
          console.log(
            `[HOST] Keyframe request from viewer ${msg.fromUserId.substring(
              0,
              8
            )}`
          );
            ...
            for (const sender of senders) {
              if (sender.track?.kind === 'video') {
                try {
                  ...
                  console.log(
                    `[HOST] Generated keyframe for viewer ${msg.fromUserId.substring(
                      0,
                      8
                    )}`
                  );
                  ...
                  console.warn(
                    `[HOST] Failed to generate keyframe for viewer ${msg.fromUserId.substring(
                      0,
                      8
                    )}:`,
                    err
  ...
  function attemptRecovery(
    ...
    if (attempts >= 3) {
      ...
      toast({
        ...
        description: `Failed to recover connection for ${connectionId.substring(
          0,
          8
        )}`,
        variant: 'destructive'
  ...
  async function createViewerConnection(viewerUserId: string) {
    ...
    console.log(
      `[HOST] Forcing H.264 for viewer ${viewerUserId.substring(0, 8)}`
    );
        ...
        try {
          ...
          if (capabilities && capabilities.codecs) {
            ...
            if (h264Codecs.length > 0) {
              ...
              h264Forced = true;
              console.log(
                `[HOST] Forced H.264 via setCodecPreferences for viewer ${viewerUserId.substring(
                  0,
                  8
                )}`
              );
          ...
          console.warn(
            `[HOST] setCodecPreferences failed for viewer ${viewerUserId.substring(
              0,
              8
            )}, will use SDP munging:`,
            err
          );
        }
      }
    ...
    if (localStreamRef.current) {
      const { qualityManager, stopMonitoring } =
    ...
    if (offer.sdp) {
      ...
      if (!h264Forced) {
        ...
        console.log(
          `[HOST] Forced H.264 via SDP munging for viewer ${viewerUserId.substring(
            0,
            8
          )}`
        );
    ...
    console.log(
      `[HOST] offer → viewer ${viewerUserId.substring(0, 8)} (h264-only=${
        h264Forced || !!offer.sdp
      })`
    );
    ...
    for (const sender of senders) {
      if (sender.track?.kind === 'video') {
        try {
          ...
          await sender.generateKeyFrame?.();
          console.log(
            `[HOST] Generated keyframe for viewer ${viewerUserId.substring(
              0,
              8
            )}`
          );
  ...
  async function renegotiateAllViewers(retryCount = 0) {
    ...
    try {
      for (const [viewerUserId, pc] of Array.from(
        viewerPcs.current.entries()
      )) {
        try {
          ...
          if (localStreamRef.current) {
            for (const track of localStreamRef.current.getTracks()) {
              if (track.kind === 'video') {
                await addVideoTrackWithSimulcast(
                  pc,
                  track,
                  localStreamRef.current
                );
          ...
          try {
            ...
            if (error.name === 'InvalidStateError') {
              ...
              if (retryCount < 3) {
                ...
                console.log(
                  `Retrying renegotiation in ${delay}ms (attempt ${
                    retryCount + 1
                  }/3)`
                );
                setTimeout(() => {
          ...
          console.error(
            `Renegotiation error for viewer ${viewerUserId}:`,
            error
          );
        }
      }
  ...
  async function goLive() {
    try {
      ...
      // Flush any viewers who joined before we had tracks
      const queued = pendingViewersRef.current;
      if (queued.size) {
        console.log('[HOST] flushing pending viewers:', queued.size);
        const ids = Array.from(queued); // ← convert Set → Array
        pendingViewersRef.current.clear(); // clear first to avoid races
        for (const id of ids) await createViewerConnection(id);
      }
              ...
              {!gameState.gameId ? (
                ...
                <div className='space-y-2'>
                  ...
                  <div className='text-xs text-muted-foreground'>
                    Phase: {gameState.data?.phase || 'N/A'} | Version:{' '}
                    {gameState.version}
                  </div>
                  <div className='flex gap-2'>
                    <Button
